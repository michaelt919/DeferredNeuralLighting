...
def _sample_texture(texture, indices, mode='bilinear', resolution=512):
    ...
    if mode == 'nearest':
        ...
        X0 = tf.maximum(tf.compat.v1.floor(X), 0)
        X1 = tf.minimum(tf.compat.v1.ceil(X), resolution-1)
        Y0 = tf.maximum(tf.compat.v1.floor(Y), 0)
        Y1 = tf.minimum(tf.compat.v1.ceil(Y), resolution-1)
        ...
        frac_indices = update_indices - tf.compat.v1.floor(update_indices)
...
def create_neural_texture_single(resolution = 512, channels = 16, init_type="glorot_uniform"):
    ...
    neural_texture = tf.compat.v1.get_variable(name='neural_texture', shape = shape, dtype=tf.float32, trainable=True, initializer=init)
    return neural_texture
...
def create_neural_texture_hierarchy(resolution = 512, channels = 16, levels = 4, init_type="glorot_uniform"):
    ...
    for level in range(levels):
        ...
        neural_texture_hierarchy.append(tf.compat.v1.get_variable(name='neural_texture_%d' % level, shape = shape, dtype=tf.float32, trainable=True, initializer=init))
...
def create_neural_texture_hierarchy_mipmap(resolution = 512, channels = 16, levels = 4, init_type="glorot_uniform"):
    ...
    neural_texture_hierarchy.append(tf.compat.v1.get_variable(name='neural_texture_%d' % 0, shape = shape, dtype=tf.float32, trainable=True, initializer=tf.initializers.glorot_uniform()))
...
def sample_texture_hierarchy_mipmap(textures, uvs):
    ...
    # Eq-3.21
    mipmap_level = 0.5 * tf.compat.v1.log(tf.maximum(dot(duvdx, duvdx), dot(duvdy, duvdy)) ) / tf.compat.v1.log(2.0)

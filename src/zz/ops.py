...
def resnet_cyclegan(inputs, output_channles, activation, prefix, args):
    ...
    def general_conv2d(inputconv, o_d=64, f_h=7, f_w=7, s_h=1, s_w=1, stddev=0.02,
                   ...
                   relufactor=0):
        with tf.compat.v1.variable_scope(name):
            conv = tf.compat.v1.contrib.layers.conv2d(
    ...
    with tf.compat.v1.variable_scope("unet_cyclegan_%s" % prefix):
        f = 7
        ks = 3
...
def avg_downsample(img):
    ...
    batch_result =  tf.nn.conv2d(input=batch_img, filters=kernel, strides=[1, 2, 2, 1], padding="SAME")
...
def compute_number_of_parameters(var_lists=tf.compat.v1.trainable_variables(), verbose=False):
    total_parameters = 0
